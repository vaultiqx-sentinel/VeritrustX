...
const App: React.FC = () => {
  ...
  const [activityLog, setActivityLog] = useState([
    ...
    { icon: Clock, text: "Global Uplink established", time: "5m" }
  ]);
  ...
  const addAuditRecord = useCallback((name: string, role: string, score: number, verdict: string) => {
    ...
    addSystemEvent('New Audit Registered', `Integrity check for ${name} completed with ${score}% trust score.`, score > 50 ? 'success' : 'critical');
  }, [addSystemEvent]);
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case 'erp-architect': return <ERPArchitect onLogicDeploy={(d) => addSystemEvent('Logic transformation', d, 'info')} />;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
              ...
              <div className={`p-8 rounded-[2.5rem] space-y-4 border ${theme === 'onyx' ? 'bg-white/5 border-white/5' : 'bg-emerald-50/20 border-emerald-50'}`}>
                 <div className="flex items-center justify-between">
                    <div>
                       <h4 className="font-black">Presentation Hibernation</h4>
                       <p className="text-[10px] text-zinc-400 uppercase font-black tracking-widest mt-1">Freezes background tasks for demos</p>
                    </div>
                    <button 
            ...
            <div className={`p-10 rounded-[3rem] flex flex-col justify-center border ${theme === 'onyx' ? 'bg-zinc-950/50 border-white/10' : 'bg-emerald-50/40 border-emerald-100'}`}>
              ...
              <p className="text-sm leading-relaxed font-medium text-zinc-400">
                VeritrustX current logic shard: <span className="text-white font-bold">{theme.toUpperCase()}</span>. All forensic captures are metadata-tagged and non-repudiable.
              </p>
            </div>
          </div>
        </div>
           ...
           <div className={`text-white rounded-[2rem] shadow-2xl p-6 w-80 border relative group hover:w-96 transition-all overflow-hidden ${theme === 'onyx' ? 'bg-zinc-900 border-white/10' : 'bg-zinc-950 border-white/5'}`}>
              <div className="flex items-center justify-between mb-4">
                 <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-60">Global Activity Ledger</span>
                 </div>
