...
const ForensicLab: React.FC<ForensicLabProps> = ({ onVerdict }) => {
  ...
  // 🛡️ HARDWARE SYNC: Ensures the video element binds to the stream correctly
  useEffect(() => {
  ...
  const startCamera = async () => {
    try {
      ...
      alert("Institutional Node Error: Please enable camera permissions and use HTTPS.");
    }
  };
  ...
  // 📸 ROBUST CAPTURE: Fixes the "Signal Weak" empty image error
  const capturePhoto = () => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (video.readyState !== 4) {
        alert("Warming up sensors. Try again in 1 second.");
        return;
      }
      ...
      if (ctx) {
        ...
        // Validation: Ensure the image isn't blank
        if (dataUrl && dataUrl.length > 5000) {
          setImage(dataUrl);
          stopCamera();
        } else {
          alert("Capture failed: Forensic node signal weak. Reposition and try again.");
        }
      }
    }
  };
  ...
  // 🧠 NEURAL UPLINK: Connects to Render backend
  const startAnalysis = async () => {
    ...
    try {
      ...
      // Force Render to wake up if it's sleeping
      ...
      if (!response.ok) throw new Error("Backend Shard Offline");

      const data = await response.json();
      setResult(data.report);

      // Save to Vault via App.tsx callback
      ...
      setResult("Forensic Link Fault: Backend mesh unreachable. Please open https://veritrustx.onrender.com/api/records in a new tab to wake up the server.");
    } finally {
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
           <div className="space-y-6">
              ...
              <div className={`relative group aspect-[3/4] rounded-[2.5rem] border-2 border-dashed transition-all flex flex-col items-center justify-center overflow-hidden bg-slate-950 ${image || isCameraActive ? 'border-red-500/50' : 'border-slate-800'}`}>
                 ...
                 {isCameraActive ? (
                   ...
                   <>
                     ...
                     <div className="absolute inset-0 flex items-center justify-center gap-6 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60">
                        <button onClick={() => setImage(null)} className="p-5 bg-red-600/20 rounded-3xl text-red-500 hover:bg-red-600/40 transition-all"><RefreshCcw size={28} /></button>
                     </div>
                   </>
                 ) : (
              ...
              <button 
                ...
                className="w-full py-6 bg-red-600 hover:bg-red-700 text-white font-black rounded-3xl flex items-center justify-center gap-3 disabled:opacity-30 transition-all shadow-2xl overflow-hidden relative group"
              >
           ...
           <div className="space-y-6">
              {result ? (
                <div className="bg-slate-950 border border-white/10 rounded-[3rem] p-10 space-y-8 animate-in slide-in-from-right-8 shadow-2xl">
                   <div className="flex items-center justify-between border-b border-white/5 pb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-white font-black uppercase">Forensic Summary</h4>
                      </div>
                ...
                <div className="h-full min-h-[500px] border-2 border-dashed border-slate-800 rounded-[3rem] flex flex-col items-center justify-center p-16 text-center bg-slate-950/20">
                   ...
                   <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest mt-4 leading-loose">Input visual evidence via camera scan or file upload to proceed.</p>
                </div>
              )}
           </div>
        </div>
