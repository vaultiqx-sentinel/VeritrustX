...
import { 
  ...
  Sun, Moon, Shield, Sparkles, Globe, MessageSquare, ExternalLink
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, currentTheme, onThemeChange }) => {
    ...
    >
      ...
      {badge && (
        <span className={`text-[8px] px-1.5 py-0.5 rounded font-black whitespace-nowrap ${
          ...
          badge === 'FINANCE' ? 'bg-indigo-600 text-white' :
          badge === 'DNA' ? 'bg-rose-600 text-white' :
          badge === '24/7' ? 'bg-zinc-900 text-white animate-pulse' :
  ...
  return (
    <aside className={`w-64 border-r h-screen flex flex-col fixed left-0 top-0 z-20 print:hidden shadow-sm transition-colors duration-500 ${currentTheme === 'onyx' ? 'bg-zinc-950 border-white/5 text-zinc-400' : 'bg-white border-emerald-50 text-zinc-600'}`}>
          ...
          return (
            <div key={category}>
              <div className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] px-4 mb-3 flex items-center justify-between">
                {category}
                <div className="w-8 h-px bg-zinc-100"></div>
              </div>
      ...
      {/* 🌈 NEURAL SPECTRUM (THEME SWITCHER) */}
      <div className={`p-4 border-t ${currentTheme === 'onyx' ? 'border-white/5 bg-black/30' : 'border-emerald-50 bg-zinc-50/50'}`}>
        ...
        <div className={`flex p-1 rounded-xl gap-1 ${currentTheme === 'onyx' ? 'bg-zinc-900' : 'bg-white border border-zinc-200'}`}>
           <button 
